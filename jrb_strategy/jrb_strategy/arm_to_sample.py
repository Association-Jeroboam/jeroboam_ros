...
class ArmToSample(Node):
    def __init__(self):
        ...
        self.pub_left_arm_goto = self.create_publisher(Pose, "left_arm_goto", 10)
    ...
    def on_sample_detected(self, msg: SampleDetectedArray):
        poses = [sample.pose for sample in msg.poses]
        left_arm_goal = poses[0]

        self.get_logger(
            f"Sample detected ! left arm GOTO {left_arm_goal.x} {left_arm_goal.y}"
        )

        self.pub_left_arm_goto(left_arm_goal)
