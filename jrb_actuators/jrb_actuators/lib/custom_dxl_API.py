...
def getModel(DXL_ID):
    for i in range (0,5) :
    	...
    	if dxl_comm_result != COMM_SUCCESS:
    	    print("Ping of ",DXL_ID,": %s" % packetHandler.getTxRxResult(dxl_comm_result))
    	elif dxl_error != 0:
    	    print("Ping of ",DXL_ID,": %s" % packetHandler.getRxPacketError(dxl_error))
    	else:
...
def setDriveMode(DXL_ID,MODE):
    writeValue(portHandler, 1, DXL_ID, 11, MODE) # XL320 : 1: wheel mode   2: joint mode   // XL430 : 1:Velocity  3:Position  4:Extended position  16:PWM
    self.driveMode=MODE
...
def writeValue(portHandler, size, ID, address, value):
    if size == 1 :
        ...
        dxl_comm_result, dxl_error = packetHandler.write3ByteTxRx(portHandler, ID, address, value)
    elif size == 4: 
        dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, ID, address, value)
    else :
        print("Incorrect size")
        return

    if dxl_comm_result != COMM_SUCCESS:
        print("Comm_result for ID",ID," address",address,": %s" % packetHandler.getTxRxResult(dxl_comm_result))
    elif dxl_error != 0:
        print("DXL error (",dxl_error,")for ID",ID," address",address,": %s" % packetHandler.getRxPacketError(dxl_error))
...
def readValue(portHandler, size, ID, address):
    if size == 1 :
        ...
        value, dxl_comm_result, dxl_error = packetHandler.read3ByteTxRx(portHandler, ID, address)
        if dxl_comm_result != COMM_SUCCESS:
            time.sleep(0.05)
            value, dxl_comm_result, dxl_error = packetHandler.read3ByteTxRx(portHandler, ID, address)
    elif size == 4: 
        value, dxl_comm_result, dxl_error = packetHandler.read4ByteTxRx(portHandler, ID, address)
        if dxl_comm_result != COMM_SUCCESS:
            time.sleep(0.05)
            value, dxl_comm_result, dxl_error = packetHandler.read4ByteTxRx(portHandler, ID, address)
    else :
    ...
    if dxl_comm_result != COMM_SUCCESS:
        ...
        print("DXL error for ID",ID," address",address,": %s" % packetHandler.getRxPacketError(dxl_error))
    else :
        return value
    return -1
...
class XL320:
    ...
    def __init__(self,DXL_ID,CW_Angle_Limit=0,CCW_Angle_Limit=1023,Moving_Speed=2047,Max_Torque=1023):
        self.ID=DXL_ID
    ...
    def setGoalPosition(self,POSITION):
        if self.CW_Angle_Limit>POSITION :
            print("Error for ID",self.ID,": Position goal (", POSITION,") < CW_Angle_Limit (",self.CW_Angle_Limit,")")
            POSITION=self.CW_Angle_Limit
        elif self.CCW_Angle_Limit<POSITION :
            print("Error for ID",self.ID,": Position goal (", POSITION,") > CCW_Angle_Limit (",self.CCW_Angle_Limit,")")
    ...
    def setDriveMode(self,MODE):
        writeValue(portHandler, 1, self.ID, 11, MODE) # XL320 : 1: wheel mode   2: joint mode   // XL430 : 1:Velocity  3:Position  4:Extended position  16:PWM
        self.driveMode=MODE
...
class XL430:
    ...
    def __init__(self,DXL_ID,CW_Angle_Limit=0,driveMode=4,CCW_Angle_Limit=4095,Moving_Speed=1023):
        self.ID=DXL_ID
        self.driveMode=driveMode
        if 1060 == getModel(DXL_ID):
            self.model_number=1060
        else :
            print("Error for ID",self.ID,": DXL with ID ",DXL_ID," does not appear to be XL430")
        self.setTorque(0)
        if(driveMode==3):
            self.setAngleLimits(CW_Angle_Limit,CCW_Angle_Limit)
            time.sleep(0.01)
    ...
    def setAngleLimits(self,CW_Angle_Limit,CCW_Angle_Limit):
        if(self.driveMode!=3):
            print("Warning for ID",self.ID,": Angle limits are not used for selected drive mode (",self.driveMode,")")
            return
    ...
    def setDriveMode(self,MODE):
        writeValue(portHandler, 1, self.ID, 10, MODE) # XL320 : 1: wheel mode   2: joint mode   // XL430 : 1:Velocity  3:Position  4:Extended position  16:PWM
        self.driveMode=MODE
...
class bras:
    ...
    def setArmPosition(self,x_sucker,y_sucker,angle_C=0,angle_D=90):
        ...
        if math.sqrt(x_sucker**2+y_sucker**2)>(h1+h2):
            print("Point trop distant pour être atteint par le bras (",x_sucker,";",y_sucker,")")
            return
        ...
        if self.isReachable('A',angle_A1) and self.isReachable('B',angle_B1) :
            ...
            print("Aucune solution trouvée pour positionner la ventouse en (",x_sucker,";",y_sucker,")")
            return
